...
{
	class Engine extends \Sys\Model {
		...
		public function addColumn($name, $type, $length, $additionalData, $comment = '')
		{
			if ($this->_currentTable == null)
				throw new Sys\Exception('addColumn must be called AFTER a table is selected in the install script');
			...
			// if we add a column on an existing table (ALTER TABLE)
			// then we add the prefix to the query ("add" column)
			if ($this->_operation == self::OP_UPDATE)
				$column->setPrefix('add');
		...
		protected function _columnsQuery($data)
		{
			$columnsQuery = array();
			if (isset($data['columns']))
			{
				foreach ($data['columns'] as $column)
				{
					...
					$identity = ($column->getIdentity() === TRUE)
						? 'AUTO_INCREMENT'
						: '';
					$columnsQuery[] = sprintf('%s %s %s %s %s %s %s %s',
						$column->getPrefix(),
						$column->getName(),
						$type,
						$unsigned,
		...
		protected function _insertsQuery($data)
		{
			$insertsQuery = array();
			if (isset($data['data']))
			{
				...
				foreach ($operations as $operation => $data)
				{
					...
					switch ($operation)
					{
						...
						case self::OP_UPDATE:
							$columnsQuery = $this->_columnsQuery($data);
							$insertsQuery = $this->_insertsQuery($data);
							$query .= sprintf('ALTER TABLE `%s` %s; %s ',
									$table,
									implode(", ", $columnsQuery),
									implode(" ", $insertsQuery));
							break;
